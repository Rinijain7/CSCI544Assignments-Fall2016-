...
for every in inputdata:
    ...
    for UTTERANCE in every:
        ...
        if i == len(every):
            newout.append("LAST UTTERANCE=True")
            ...
            continueSpeak=1
        if continueSpeak > 1:
            newout.append("CONTINUE SPEAK="+str(continueSpeak))
        continueSpeak += 1

        if UTTERANCE[2]:
            for other in range(0, len(UTTERANCE[2])):
                newout.append("token=TOKEN_" + UTTERANCE[2][other][0])
                newout.append("pos=POS_" + UTTERANCE[2][other][1])
                if other < (len(UTTERANCE[2]) - 1):
                    newout.append("token_bigram=TOKEN_BIGRAM_" + UTTERANCE[2][other][0] + "_" + UTTERANCE[2][other + 1][0])
                    newout.append("pos_bigram=POS_BIGRAM_" + UTTERANCE[2][other][1] + "_" + UTTERANCE[2][other + 1][1])
...
for every in testdata:
    ...
    for UTTERANCE in every:
        ...
        if j == len(every):
            newout1.append("LAST UTTERANCE=True")
            ...
            continueSpeak=1
        if continueSpeak > 1:
            newout1.append("CONTINUE SPEAK="+str(continueSpeak))
        continueSpeak += 1
        if UTTERANCE[2]:
            for other in range(0, len(UTTERANCE[2])):
                newout1.append("token=TOKEN_" + UTTERANCE[2][other][0])
                newout1.append("pos=POS_" + UTTERANCE[2][other][1])
                if other < (len(UTTERANCE[2]) - 1):
                    newout1.append("token_bigram=TOKEN_BIGRAM_" + UTTERANCE[2][other][0] + "_" + UTTERANCE[2][other + 1][0])
                    newout1.append("pos_bigram=POS_BIGRAM_" + UTTERANCE[2][other][1] + "_" + UTTERANCE[2][other + 1][1])
...
# with open('result.txt','w') as foutput:
